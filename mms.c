static char help[] = "Final, All-in-One Parallel 2D Heat Solver with Physics/MMS modes, Explicit/Implicit schemes, HDF5 Restart, and VTK output.\n\n";
...
int main(int argc, char **args) {
    ...
    PetscInt            nx = 51, ny = 51, max_steps = 200, start_step = 0;
    ...
    if (flg_restart) {
        ...
        for (PetscInt j=info.ys; j<info.ys+info.ym; j++) {
            for (PetscInt i=info.xs; i<info.xs+info.xm; i++) {
                PetscReal x=i*dx, y=j*dy;
                if (run_type == RUN_PHYSICS) {
                    if (x>=0.4&&x<=0.6&&y>=0.4&&y<=0.6) u_local[j][i] = 100.0; else u_local[j][i] = 0.0;
                } else { // RUN_MMS
                    u_local[j][i] = sin(PETSC_PI * x) * sin(PETSC_PI * y);
                }
            }
        }
        ierr = DMDAVecRestoreArray(da, u, &u_local); CHKERRQ(ierr);
    ...
    for (PetscInt step = start_step; step < max_steps; ++step) {
        ...
        if (run_type == RUN_MMS) {
            DMDALocalInfo info; ierr = DMDAGetLocalInfo(da, &info); CHKERRQ(ierr);
            PetscScalar **f_local; ierr = DMDAVecGetArray(da, f_source, &f_local); CHKERRQ(ierr);
            PetscReal dx = 1.0/(nx-1), dy = 1.0/(ny-1);
            for (PetscInt j = info.ys; j < info.ys + info.ym; j++) {
                for (PetscInt i = info.xs; i < info.xs + info.xm; i++) {
                    PetscReal x = i * dx, y = j * dy;
                    f_local[j][i] = (2.0*diffusivity*PETSC_PI*PETSC_PI - 1.0) * sin(PETSC_PI*x) * sin(PETSC_PI*y) * exp(-current_time);
                }
            }
    ...
    if (run_type == RUN_MMS) {
        ...
        for (PetscInt j = info.ys; j < info.ys + info.ym; j++) {
            for (PetscInt i = info.xs; i < info.xs + info.xm; i++) {
                PetscReal x = i * dx, y = j * dy;
                uexact_local[j][i] = sin(PETSC_PI * x) * sin(PETSC_PI * y) * exp(-final_time);
            }
        }
